...
def main():
    ...
    X_train, model_pca = Optimise.apply_pca(X_train, 60)

    scale_time = time.time() - start_scale
    print (f"Scaling to 60 for {training_file}:  {scale_time:.5f} seconds")
    print()
    ...
    print (f"transform and encrypt {test_file}:  {scale_time4:.5f} seconds")
    ...
    run_svm_inference(
            ...
            label = (" Best performant SVM combination"),
        )
